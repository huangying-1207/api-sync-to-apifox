...
export class ApiScanner {
  ...
  /**
   * 收集 DTO 扫描范围：变更文件 + Controller 引用的 dto/entity/vo 类
   */
  private collectDtoScanScope(sourcePath: string): string[] | undefined {
    ...
    for (const file of this.changedFiles) {
      if (!/Controller\.java$/i.test(file) || !fs.existsSync(file)) {
        continue;
      }
      ...
      for (const match of importMatches) {
        ...
        if (!dtoPackagePattern.test(classPath)) {
          continue;
        }
        ...
        for (const candidate of candidates) {
          if (fs.existsSync(candidate)) {
            scopedFiles.add(path.normalize(candidate));
          }
        }
      }
    }
  ...
  /**
   * 扫描 Java 类文件，提取 DTO 字段定义
   */
  scanJavaClasses(sourcePath: string, scopedFiles?: string[]): any {
    const classSchemas: any = {};
    const dtoScope = scopedFiles ?? this.collectDtoScanScope(sourcePath);
    ...
    try {
      ...
      if (dtoScope && dtoScope.length > 0) {
        ...
        javaFiles = globSync(normalizedSourcePath + '/**/*.java');
  ...
  async scanCodeByFramework(sourcePath: string, framework: string): Promise<ApiInfo[]> {
    ...
    if (framework === 'springboot') {
      this.scanJavaClasses(sourcePath, this.collectDtoScanScope(sourcePath));
    }
  ...
  setChangedFiles(files: string[]): void {
    this.changedFiles = files
      ...
      .filter((file) => !isTestOrNonApiSourceFile(file));
  }

  /**
   * 将扫描范围限定为同步计划中的 Controller 文件，避免 sync 时全量扫描
   */
  scopeToPlanChangedFiles(changedFiles: string[]): void {
    ...
    if (controllerFiles.length > 0) {
      this.setChangedFiles(controllerFiles);
      return;
    }
